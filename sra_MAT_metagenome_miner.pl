...
# Requires sratoolkit, bwa, samtools, ITSx [optional]

# Expected input is a fasta file of DNA sequences
# A list of SRA data in a text format with one experiment (acc. num.) per line.
...
# Change to directory for the fastq data
...
$cmd = 'bwa index -p targets_bwa_reference ' . $inputfasta;
print $cmd;
system($cmd); 

# Extract sequence names from fasta file
my @seqnames;
open (INPUTFASTA, $inputfasta) or die $!;
...
print "\n$numseqs query sequences parsed. End indexing.\n\n";
# close the file
close INPUTFASTA;
...
open (OUTPUTTABLE, ">sra_MAT_miner_output.txt");
